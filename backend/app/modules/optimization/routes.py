...
async def get_user_tasks(user: UserDep, optimization_service: OptimizationServiceDep):
    ...
    tasks = await optimization_service.get_user_tasks(user.id)
    return {"tasks": tasks}


@op_router.post("")
async def optimize(user: UserDep, optimization_service: OptimizationServiceDep, data: Optimize):
    result = await optimization_service.optimize(user_id=user.id, resume_id=data.resume_id, job_id=data.job_id)
    return result
